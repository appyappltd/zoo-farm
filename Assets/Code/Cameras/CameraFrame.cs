...
namespace Cameras
{
    public class CameraFrame
    {
        ...
        {
            ProjectCross();
            float closestEnter = float.MaxValue;
...
#if DEBUG
            Drawer.DrawSphere(point, 1f, Color.red, 0.01f);
#endif
            return point;
        }
...
#if DEBUG
                Debug.DrawRay(_edgeRays[i].origin, _edgeRays[i].direction * 5, _colors[i]);
#endif
            }
        }
...
#if DEBUG
                Debug.DrawRay(_edgeRays[i].origin, _edgePlanes[i].normal * 5, _colors[i]);
#endif
            }
        }
    }
